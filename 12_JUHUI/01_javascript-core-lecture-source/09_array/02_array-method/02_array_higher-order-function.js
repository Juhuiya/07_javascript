...
for (let i = 0; i < 10; i++) {
  // floor 소수점 이하 잘라내겠다.
  numbers[i] = Math.floor(Math.random() * 100) + 1; // 1부터 100까지의 랜덤한 숫자 뽑아서 넣을 거임
}

console.log(`문자 정렬 전 numbers : ${numbers}`);
numbers.sort(); // 정렬(오름차순)
console.log(`문자 정렬 후 numbers : ${numbers}`); // 문자 정렬렬

// sort의 기본 정렬은 문자 정렬이므로 필요하다면 정렬 기준을 콜백함수를 전달해야한다.
numbers.sort((a, b) => a - b); // 숫자 정렬렬
...
// Array.prototype.filter : 배열 요소 전체를 대상으로 콜백함수 호출 후 반환값이 true인 요소로만 구성된 새로운 배열 반환
console.log(numbers);
const odds = numbers.filter((item) => item % 2); // 나머지가 1이 true여서
console.log(odds); // 1,3,5가 출력됨
...
// find : 배열을 순회하며 각 요소에 대하여 인자로 주어진 콜백함수를 실행하여 그 결과가 참인 첫번재 요소를 반환
// 참인 첫번째 요소가 존재하지 않는다면 undefined를 반환
